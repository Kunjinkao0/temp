<template>
    <div class="container cu-flex-y">
        <Breadcrumb :items="['menu.rescue', 'menu.repair.repairOrder']" />
                        ...
                        <a-col :flex="5">
                            <a-form :model="formModel" :label-col-props="{ span: 6 }" :wrapper-col-props="{ span: 18 }"
                                ...
                                <a-row :gutter="16">
                                    <a-col :span="8">
                                        <!-- 电梯名称 -->
                                        <a-form-item field="elevatorName" :label="$t('menu.repair.elevatorName')"
                                            label-col-flex="60px">
                                            <a-input class="input-search" v-model="formModel.elevatorName"
                                                :placeholder="$t('menu.repair.elevatorName.placeholder')" allow-clear/>
                                        </a-form-item>
                                    </a-col>
                                    <a-col :span="8">
                                        <!-- 维保单位 -->
                                        <a-form-item field="maintainUnit" :label="$t('menu.repair.maintainUnit')"
                                            label-col-flex="60px">
                                            <a-input class="input-search" v-model="formModel.maintainName"
                                                :placeholder="$t('menu.repair.maintainUnit.placeholder')" allow-clear/>
                                        </a-form-item>
                                    </a-col>
                                    <a-col :span="8">
                                        <!-- 使用单位 -->
                                        <a-form-item field="useUnit" :label="$t('menu.repair.useUnit')"
                                            label-col-flex="60px">
                                            <a-input class="input-search" v-model="formModel.propertyName"
                                                :placeholder="$t('menu.repair.useUnit.placeholder')" allow-clear/>
                                        </a-form-item>
                                    </a-col>
                                </a-row>
                            </a-form>
                                ...
                                <span v-for="(item, index) in stateList" :key="index">
                                    <a-tag checkable color="arcoblue" :checked="item.checked" size="large"
                                        @check="workOrderStateCheck(item.state, $event)" bordered>{{
                                                $t(`menu.repair.state.${item.state}`)
                                        }}</a-tag>
                                </span>
                            </a-space>
                        </a-col>
                        ...
                        <template #state="{ record }">
                            <span v-if="record.state === 1">
                                ...
                                <a-badge text="NEW" :offset="[13, 5]" :dot-style="{width:'40px',height:'15px',fontSize:'8px',lineHeight:'12px'}">
                                    <a-tag color="orange">{{ $t(`menu.repair.state.${record.state}`) }}</a-tag>
                                </a-badge>
                            </span>
                            <span v-else-if="record.state === 2">
                                <!-- 已接单 -->
                                <a-tag color="cyan">{{ $t(`menu.repair.state.${record.state}`) }}</a-tag>
                            </span>
                            <span v-else-if="record.state === 3">
                                <!-- 已到场 -->
                                <a-tag color="blue">{{ $t(`menu.repair.state.${record.state}`) }}</a-tag>
                            </span>
                            <span v-else>
                                <!-- 已完成 -->
                                <a-tag>{{ $t(`menu.repair.state.4,7`) }}</a-tag>
                            </span>
                        </template>
...
import { getEleWorkList, QueryRecordParams, QueryParams } from '@/api/repair/repairOrder';
...
const columns = computed<TableColumnData[]>(() => [
    ...
    {
        title: t('menu.repair.elevatorName'),
    ...
    {
        title: t('menu.repair.maintainUnit'),
    ...
    {
        title: t('menu.repair.planRepairTime'),
        dataIndex: 'planDate',
        ellipsis: true,
        tooltip: true,
        align: 'left',
