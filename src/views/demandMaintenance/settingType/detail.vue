...
import { parseInt } from 'lodash';
  ...
  const formModel = ref({
    ...
    description: '',
    iotDeviceStatus: 0,
    yearInspectionStatus: 0,
    useServiceLife: 0,
    maintainAddress: '',
    safetyAccidentYears: 0,
    legalResponsibilityStatus: 0,
    maintenancelicenseStatus: 0,
    isEleUseAge: 0,
  ...
  const getDetail = async () => {
    ...
    try {
      ...
      const detail = await querySettingTypeDetail(param);
      formModel.value.id = idParam;
      formModel.value.name = detail.data.name;
      formModel.value.description = detail.data.description;
      formModel.value.useServiceLife = detail.data.useServiceLife;
      formModel.value.maintainAddress = detail.data.maintainAddress;
      formModel.value.safetyAccidentYears = detail.data.safetyAccidentYears;
      formModel.value.inspectionPassRate = detail.data.inspectionPassRate;
      formModel.value.inspectionPassYears = detail.data.inspectionPassYears;
      formModel.value.matainInterval = detail.data.matainInterval;
      formModel.value.iotDeviceStatus = detail.data.iotDeviceStatus;
      formModel.value.yearInspectionStatus = detail.data.yearInspectionStatus;
      formModel.value.legalResponsibilityStatus = detail.data.legalResponsibilityStatus;
      formModel.value.maintenancelicenseStatus = detail.data.maintenancelicenseStatus;
      formModel.value.isEleUseAge = detail.data.useServiceLife;
      formModel.value.needUploadMedia = detail.data.needUploadMedia;
    } catch (err) {
  ...
  const savaSettingType = async () => {
    setLoading(true);
    try {
      await updateSettingType(formModel.value);
      // 这得提示用接口返回的文字
