<template>
    <div class="container">
        <Breadcrumb :items="['menu.maintanance', 'menu.maintenance.statistics']" />
        <a-card class="general-card" :title="$t('menu.maintenance.statistics.form.title')">
            <template #title>
            <span class="cu-fontWeight700 cu-font16"> {{$t('menu.maintenance.statistics.form.title')}} </span>
            </template>
            <a-row>
                <a-col :flex="5">
                <a-form
                ...
                >
                    <a-row :gutter="16">
                        <a-col :span="8">
                            <a-form-item
                            field="ownerId"
                            :label="$t('menu.maintenance.statistics.form.owner')"
                            >
                        ...
                        <a-col :span="8">
                            <a-form-item
                            field="mainUnitId"
                            :label="$t('menu.maintenance.statistics.form.mainUnit')"
                            >
                                <a-select v-model="formModel.maintainIds" :placeholder="$t('menu.maintenance.statistics.form.mainUnit.placeholder')"
                                          :trigger-props="{ autoFitPopupMinWidth: true }" class="input-search" :tree-check-strictly="true" allow-clear allow-search>
                        ...
                        <a-col :span="8">
                            <a-form-item
                            field="useUnitId"
                            :label="$t('menu.maintenance.statistics.form.useUnit')"
                            >
                                <a-select v-model="formModel.propertyIds" :placeholder="$t('menu.maintenance.statistics.form.useUnit.placeholder')"
        ...
        <a-grid :cols="24" :col-gap="16" :row-gap="16" style="margin-top: 16px">
            <a-grid-item
            :span="{ xs: 24, sm: 24, md: 24, lg: 24, xl: 10, xxl: 10 }"
            >
                <UnderMaintenanceOrder ref="underMaintenanceOrderRef" :parent-params="formModel"/>
            </a-grid-item>
            <a-grid-item
            :span="{ xs: 24, sm: 24, md: 24, lg: 24, xl: 14, xxl: 14 }"
            >
                <AfterMaintenanceOrder  ref="afterMaintenanceOrderRef" :parent-params="formModel"/>
            </a-grid-item>
      </a-grid>
      <a-grid :cols="24" :col-gap="16" :row-gap="16" style="margin-top: 16px">
            <a-grid-item
            ...
            >
                <OvertimeOrder ref="overtimeOrderRef" :parent-params="formModel"/>
            </a-grid-item>
            <a-grid-item
            :span="{ xs: 24, sm: 24, md: 24, lg: 24, xl: 12, xxl: 12 }"
            >
                <OvertimeStatitstics ref="overtimeStatitsticsRef" :parent-params="formModel" />
            </a-grid-item>
      </a-grid>
    </div>
</template>
<script lang="ts" setup>
    ...
    import UnderMaintenanceOrder from './components/under-maintenance-order.vue'
    import AfterMaintenanceOrder from './components/after-maintenance-order.vue'
    import OvertimeOrder from './components/overtime-order.vue'
    import OvertimeStatitstics from './components/overtime-statitstics.vue'

    const workStatisticsRef = ref();
    const underMaintenanceOrderRef = ref();
    const afterMaintenanceOrderRef = ref();
    const overtimeOrderRef = ref();
    const overtimeStatitsticsRef = ref()
    ...
    const search = () => {
        ...
        underMaintenanceOrderRef.value.parentSearch();
        afterMaintenanceOrderRef.value.parentSearch();
        overtimeOrderRef.value.parentSearch();
        overtimeStatitsticsRef.value.fetchData()
    }
