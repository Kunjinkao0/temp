<template>
    <a-spin :loading="loading" style="width: 100%">
      ...
      >
        <template #title>
          <span class="cu-fontWeight700 cu-font16"> {{$t('menu.maintenance.statistics.overtime.title')}} </span>
        </template>
        <a-space direction="vertical" :size="10" fill>
            ...
            >
                <template #elevatorNumSolt="{ record }">
                    <span class="score-style">{{record.elevatorNum}}</span>
                </template>
  ...
  <script lang="ts" setup>
    ...
    import { getMainOverRanking } from '@/api/maintenance/maintenanceStatistics';
    ...
    const columns = computed<TableColumnData[]>(() => [
        ...
        {
        title: t('menu.maintenance.statistics.elevatorNum'),
        dataIndex: 'eleNum',
        ellipsis: true,
        align:'center',
        headerCellClass: 'body-cell-class'
    ...
    const fetchData = async (
        ...
        setLoading(true);
        try {
          const { data } = await getMainOverRanking(params);
